...
def read_yahoo_summary(stock,out=True,att=5,default="N/A"):
    ...
    if tmp_next_ed == None: erg["next_earnings_date"] = default
        ...
        if tmp_next_ed not in [default,"N/A",None,""]:
...
def readBenzingaNews(ticker, date_from=None, date_to=None, maxCount=100, limit4PM=True, api=None):
    """
    ...
    for idx, elem in enumerate(stories):
        for pos in ["channels","stocks","tags"]:
            listStories = []
            for elem2 in elem[pos]:
                listStories.append(elem2["name"])    
            if pos in ["channels","tags"]:
                stories[idx][pos] = ", ".join(listStories)
            else:
                stories[idx][pos] = listStories
...
def readYFPrices(ticker,startDate=None,endDate=None):
    ...
    if startDate != None and endDate != None:
        ...
        startDT = datetime.strptime("2010-01-01", "%Y-%m-%d")
        endDT = datetime.strptime(endDate, "%Y-%m-%d")      
        erg = yf.download(ticker,start=startDT,end=endDT) 
    elif startDate == None and endDate == None:
        startDT = datetime.strptime("2010-01-01", "%Y-%m-%d")
